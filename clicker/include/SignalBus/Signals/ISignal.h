...
#define REGISTER_SIGNAL(signal_type)                                                                      \
    ...
    template void SignalBus::subscribe<signal_type>(std::function<void(const signal_type &)> handler);    \
    template void SignalBus::unsubscribe<signal_type>(std::function<void(const signal_type &)> handler);  \
